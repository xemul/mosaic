...
static int thin_init_base(char *t_name, char *dev, char *fs, unsigned long sz)
{
	...
	id = thin_get_id(dev, t_name, NULL, true);
	if (id < 0)
		return -1;
...
static void del_thin(struct tessera *t, char *age)
{
	struct thin_tessera *tt = t->priv;
...
static int grow_thin(struct tessera *t, char *base_age, char *new_age)
{
	struct thin_tessera *tt = t->priv;
	int base_id, id;
	char cmd[1024];
	...
	sprintf(cmd, "dmsetup create mosaic-%s-%s --table \"0 %lu thin %s %d\"",
...
struct iag {
	int (*cb)(struct tessera *t, char *age, void *);
	struct tessera *t;
	void *arg;
};
